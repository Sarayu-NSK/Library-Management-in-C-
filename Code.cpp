...
public:
	...
	void setDate(Date temp) // overloading of a function
	{
		dd = temp.dd + 15;
		mm = temp.mm;
		yy = temp.yy;
...
void Menu :: mainMenu()// scope resolution operator ::
{
...
{
	fstream ofp;//A combination of ofstream - w and ifstream- r: creates, reads, and writes to files
	ofp.open("bookDatabase.dat", ios :: out | ios :: binary | ios :: app); // ios - input output stream - baseclass
// out - File open for writing; binary - Operations are performed in binary mode rather than text; app - append
	if(!ofp)
	{
		cerr<<"Unable to open file"<<endl; // cerr - (character error) used to display error msgs
		return;
	}
...
{
	fstream file;//A combination of ofstream - w and ifstream- r: creates, reads, and writes to files
	file.open("bookDatabase.dat", ios :: binary | ios :: in | ios :: out | ios :: ate);
//in - input,File open for reading; ot - output,File open for writing; ate - at end,output position starts at the end of the file.
	if (!file)
	{
	...
	file.seekg(0, ios :: beg); //seekg - allows you to seek to an arbitrary position in a file
//ios :: beg - This indicates that the location is the number of bytes from the beginning of the file
	...
	while (file.read((char*)&book, sizeof(BookData)))
	{
		if(strcmp(book.title, title) == 0)
		{
			...
			//seekp - set the position of the pointer in the output sequence with the specified position
...
{
	fstream file;
	file.open("bookDatabase.dat", ios :: binary | ios :: in);// in - read mode
...
void Student ::  inputDetails()
{
	...
	ofp.open("studentDatabase.dat", ios :: out | ios:: app | ios :: binary);// out - write; in - read
...
{
	fstream fp;
	fp.open("studentDatabase.dat",ios :: binary | ios :: in | ios :: out | ios :: ate);// ate = cursor at end
